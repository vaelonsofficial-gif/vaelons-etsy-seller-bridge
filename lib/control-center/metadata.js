...
import { detectUnverifiedClaims } from './claims.js';
...
function newlyIntroducedUnsupportedClaims(original, proposed) {
  const before = new Set(detectUnverifiedClaims(original).map((claim) => claim.key));
  return detectUnverifiedClaims(proposed)
    .filter((claim) => !before.has(claim.key))
    .map((claim) => claim.key);
}

function unsupportedClaimsInMetadata(metadata) {
  return detectUnverifiedClaims([
    metadata?.title,
...
export function validateMetadataProposal(original, input = {}, options = {}) {
  const proposed = {
  ...
  const newlyIntroducedClaims = newlyIntroducedUnsupportedClaims(
    original.description,
    proposed.description
  );

  const unsupportedClaims = options.strictClaims
    ? unsupportedClaimsInMetadata(proposed)
    : newlyIntroducedClaims;

  for (const claim of unsupportedClaims) {
    errors.push(options.strictClaims
      ? `unsupported_claim_present:${claim}`
      : `unsupported_claim_added:${claim}`);
  }
  ...
  return {
    ...
    qa: {
      ...
      strict_claims: options.strictClaims === true,
      unsupported_claims_added: newlyIntroducedClaims,
      unsupported_claims_present: unsupportedClaims,
      etsy_modified: false
    }
  };
}
