...
import { auditCreativeListing, summarizeCreativeAudit } from './creative-audit.js';
import { asListingId } from './metadata.js';
import { resolveContentPolicy } from './content-policy.js';
...
export async function fetchListingDetail(listingId) {
  ...
  const [identity, listing, imagePage, contentPolicy] = await Promise.all([
    getVerifiedShopIdentity(),
  ...
  const creativeAudit = auditCreativeListing(normalized, normalizedImages);

  return {
    ...normalized,
    images: normalizedImages,
    creative_audit: creativeAudit,
...
function normalizeListing(listing) {
  ...
  const normalizedImages = sortedImages(listing?.images).map(normalizeImage);
  const normalized = {
  ...
  return {
    ...
    creative_preview: summarizeCreativeAudit(auditCreativeListing(normalized, normalizedImages))
  };
}
...
export async function getDashboardSnapshot() {
  ...
  const summary = catalog.listings.reduce((acc, listing) => {
    ...
    if (listing.creative_preview.status === 'BLOCKED') acc.creative_blocked_count += 1;
    if (listing.creative_preview.image_count < 10) acc.role_capacity_short_count += 1;
    return acc;
  }, {
    visual_review_count: 0,
    claim_review_count: 0,
    repair_or_blocked_count: 0,
    creative_blocked_count: 0,
    role_capacity_short_count: 0
  });
