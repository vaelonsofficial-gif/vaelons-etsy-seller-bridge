...
function proposalHash(listingId, beforeHash, proposed, strictClaims) {
  return createHash('sha256')
    .update(JSON.stringify({ listingId, beforeHash, proposed, strictClaims: strictClaims === true }))
    .digest('hex');
}
...
export function publicAction(action) {
  ...
  return {
    ...
    generation_id: action.generation_id || null,
...
export async function prepareMetadataAction({
  ...
  if (expectedBeforeHash && beforeHash !== expectedBeforeHash) {
    const error = new Error('Listing içerik hazırlanırken değişmiş; güvenliğiniz için yeni içerik gerekli');
    error.status = 409;
    error.code = 'LISTING_CHANGED_DURING_GENERATION';
    throw error;
  }

  const validation = validateMetadataProposal(
    before,
  ...
  const action = {
    schema_version: 1,
    id: randomUUID(),
    idempotency_key: proposalHash(id, beforeHash, validation.proposed, strictClaims),
...
export async function executeMetadataAction({ actionId, approval }) {
  ...
  try {
    ...
    const executionValidation = validateMetadataProposal(
      current,
      action.proposed,
      { strictClaims: true }
    );
    if (!executionValidation.valid) {
      await updateAction(action.id, {
        status: 'BLOCKED',
        block_reason: 'execution_validation_failed',
        validation_errors: executionValidation.errors
      });
      await appendActionEvent(action.id, {
        type: 'EXECUTION_BLOCKED',
        status: 'BLOCKED',
      ...
      const error = new Error('Taslak son yayın güvenlik kontrolünden geçemedi');
      error.status = 409;
      error.code = 'EXECUTION_VALIDATION_FAILED';
      throw error;
    }
