...
export function classifyGenerationError(error) {
  ...
  if (operationalCodes.has(error?.code)) {
    ...
    classified.status = Number(error.status || error.statusCode) || 500;
    classified.providerCode = null;
    return classified;
  }

  const providerMessage = String(error?.message || '');
  let code = 'CONTENT_MODEL_FAILED';
  let message = 'İçerik motoru isteği tamamlayamadı. Üretim kaydı saklandı; güvenle yeniden denenebilir.';
  ...
  if (/valid credit card|add a card|unlock your free credits/i.test(providerMessage)) {
    ...
    status = 402;
    message = 'Vercel AI Gateway ödeme yöntemi doğrulaması bekliyor. Bu hesap-sahibi adımı tamamlanmadan içerik üretimi başlatılamaz.';
  } else if (/unauthorized|authentication|api key|oidc|forbidden/i.test(providerMessage)) {
    code = 'GATEWAY_AUTH_FAILED';
    status = 503;
    message = 'Vercel AI Gateway kimlik doğrulaması başarısız oldu. Etsy’de hiçbir değişiklik yapılmadı.';
  } else if (/rate.?limit|too many requests/i.test(providerMessage)) {
    code = 'GATEWAY_RATE_LIMITED';
    status = 429;
    message = 'İçerik motoru geçici istek sınırına ulaştı. Kayıt korundu; biraz sonra yeniden denenebilir.';
  } else if (/timeout|timed out|deadline|aborted/i.test(providerMessage)) {
    code = 'CONTENT_GENERATION_TIMEOUT';
    status = 504;
    message = 'İçerik hazırlama süresi güvenli zaman sınırını aştı ve durduruldu. Etsy’de hiçbir değişiklik yapılmadı.';
  } else if (/model.+not found|no such model/i.test(providerMessage)) {
    code = 'CONTENT_MODEL_UNAVAILABLE';
    status = 503;
    message = 'Seçili içerik modeli şu anda kullanılamıyor. Model ayarı kontrol edilmelidir.';
  }

  const classified = new Error(message);
  classified.code = code;
  classified.status = status;
  classified.providerCode = error?.code || null;
  return classified;
}
