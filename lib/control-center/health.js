...
import { getContentPolicy, resolveContentPolicy } from './content-policy.js';
...
export async function getSystemHealth() {
  ...
  const [token, identity, persistence, content] = await Promise.allSettled([
    getTokenStatus(),
  ...
  const contentPolicy = content.status === 'fulfilled' ? content.value : getContentPolicy();
