...
export function auditListing(listing) {
  ...
  const unverifiedClaims = detectUnverifiedClaims(listing.description);
  const rawTrust = trustScore(listing.description);
  const trust = unverifiedClaims.length > 0 ? Math.min(rawTrust, 50) : rawTrust;
  ...
  if (unverifiedClaims.length > 0) {
    findings.push(`Ürün verisiyle doğrulanması gereken iddialar: ${unverifiedClaims.map((claim) => claim.label).join(', ')}`);
  }
  ...
  return {
    ...
    claim_review_required: unverifiedClaims.length > 0,
